...
This service replaces the functionality of `recruitment_ad_search.py`, which previously searched for URLs and saved them to CSV files. The new service instead publishes URLs directly to a message queue for processing.
...
This service replaces the functionality of `recruitment_researcher.py`, which previously processed URLs from CSV files. The new service consumes URLs directly from the message queue.
...
- Provides message persistence to prevent data loss
...
Both services are implemented using FastAPI, a modern web framework for building APIs with Python:
...
- The URL Discovery Service publishes URLs to the "recruitment_urls" queue
- The URL Processing Service consumes URLs from the queue
- Messages include metadata such as search ID and timestamp
- Failed processing attempts are tracked and can be retried

### Neo4j Database Connection

The URL Processing Service connects to Neo4j:
- Uses the Neo4j Python driver for database operations
- Supports both local Neo4j Desktop instances and remote servers
- Implements transaction-based operations for data integrity
- Stores structured data extracted from recruitment advertisements
...
3. **Maintainability**: Services can be updated and deployed independently
4. **Elimination of CSV Files**: Data flows directly between services without intermediate files
5. **Asynchronous Processing**: URLs can be processed in the background without blocking
6. **Better Error Handling**: Failed processing can be retried automatically
7. **API-First Approach**: Services expose well-defined APIs for integration with other systems

...
class RecruitmentDatabase:
    ...
    def __init__(self, db_path: Optional[str] = None) -> None:
        ...
        """
        self.db_path = db_path or os.getenv("RECRUITMENT_PATH", "databases/recruitment.db")
        if not self.db_path:
    ...
    def insert_job(self, job_data):
        ...
        cursor.execute("""
            INSERT INTO jobs (
                ...
                salary_period, location, job_type, experience_level
            ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            title, description, salary_min, salary_max, salary_currency,
            salary_period, location, job_type, experience_level
        ...
        for skill in skills:
            ...
            self.link_job_skill(job_id, skill_id)
            
        # Insert attributes
        for attribute in attributes:
            attribute_id = self.insert_attribute(attribute)
            self.link_job_attribute(job_id, attribute_id)
            
        # Insert industries
        for industry in industries:
            industry_id = self.insert_industry(industry)
            self.link_job_industry(job_id, industry_id)
    ...
    def link_job_attribute(self, job_id: int, attribute_id: int) -> bool:
        """
        ...
        """
        with self._execute_query(query, (job_id, attribute_id)) as cursor:
            if cursor.rowcount > 0:
                self.logger.info(f"Linked job {job_id} with attribute {attribute_id}")
                return True
            else:
                self.logger.info(f"Job {job_id} already linked with attribute {attribute_id}")
                return False
    ...
    def insert_industry(self, name: str) -> int:
        ...
        Returns:
            The ID of the inserted industry
        """
        query = """
            INSERT OR IGNORE INTO industries (name, created_at, updated_at)
            VALUES (?, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
        """
        with self._execute_query(query, (name,)) as cursor:
            if cursor.rowcount > 0:
                industry_id = cursor.lastrowid
                self.logger.info(f"Inserted industry: {name} with ID: {industry_id}")
                return industry_id
            else:
                # Industry already exists, get its ID
                query = "SELECT id FROM industries WHERE name = ?"
                with self._execute_query(query, (name,)) as cursor:
                    row = cursor.fetchone()
                    if row:
                        return row[0]
                    raise DatabaseError(f"Failed to insert or retrieve industry: {name}")

    def link_job_industry(self, job_id: int, industry_id: int) -> bool:
        """
        Link a job with an industry.
        ...
        """
        query = """
            INSERT OR IGNORE INTO job_industries (job_id, industry_id) VALUES (?, ?)
        """
        with self._execute_query(query, (job_id, industry_id)) as cursor:
            if cursor.rowcount > 0:
                self.logger.info(f"Linked job {job_id} with industry {industry_id}")
                return True
            else:
                self.logger.info(f"Job {job_id} already linked with industry {industry_id}")
                return False

    def get_job_industries(self, job_id: int) -> List[Tuple[int, str]]:
        """
        Get all industries for a job.

        Args:
            job_id: The job ID

        Returns:
            List of tuples (industry_id, industry_name)
        """
        query = """
            SELECT i.id, i.name
            FROM industries i
            JOIN job_industries ji ON i.id = ji.industry_id
            WHERE ji.job_id = ?
        """
        with self._execute_query(query, (job_id,)) as cursor:
    ...
    def get_job(self, job_id):
        ...
        cursor = self.conn.cursor()
        cursor.execute("""
            SELECT id, title, description, salary_min, salary_max, salary_currency,
                   salary_period, location, job_type, experience_level, status,
        ...
        # Get skills
        skills = self.get_job_skills(job_id)
        
        # Get attributes
        attributes = self.get_job_attributes(job_id)
        
        # Get industries
        industries = self.get_job_industries(job_id)
        ...
        job_data = {
            ...
            'created_at': job[11],
            'updated_at': job[12],
            'skills': [{'id': s[0], 'name': s[1]} for s in skills],
            'attributes': [{'id': a[0], 'name': a[1]} for a in attributes],
            'industries': [{'id': i[0], 'name': i[1]} for i in industries]
        }
        
        return job_data

    def get_jobs(self, limit: int = 100, offset: int = 0, filters: Dict[str, Any] = None) -> List[Dict[str, Any]]:
        """
        Get multiple jobs with their associated data.
        
        Args:
            limit: Maximum number of jobs to return
            offset: Number of jobs to skip (for pagination)
            filters: Dictionary of filters to apply (e.g., {'status': 'active', 'job_type': 'full-time'})
